...
class nusoap_base {
	...
	*/
	function isArraySimpleOrStruct($val) {
		...
		$this->debug("in isArraySimpleOrStruct: keys: " . implode(', ', $keyList));
	...
	*/
	function setEncoding($enc='gzip, deflate') {
		if (function_exists('gzdeflate')) {
			...
			// Change by weke: since set_magic_quotes_runtime is deprecated next line was commented out
	...
	function getResponse(){
	  ...
	  if ($this->io_method() == 'socket') {
		...
		do {
			...
			$strlen = 0;
// Hack Xtra - 2009-12-31 some strato servers are buggy and give feof() === true even if there is no eof.
// we work around this by moving the feof check below.
//		    while (($strlen < $content_length) && (!feof($this->fp))) {
		    while (($strlen < $content_length)) {
				...
				$strlen += $tmplen;
				$data .= $tmp;
// Hack Xtra - 2009-12-31 some strato servers are buggy and give feof() === true even if there is no eof (there is still some data to read).
// we work around this by moving the feof check here and only using it, when we have no content length.
				if(($strlen == $content_length) || (($content_length==2147483647) && feof($this->fp)) || $tmplen===0)
					break;
			}
...
// Had to remove this as it collides with our meta class. (c.schiffler 2009-09-04)
